...
// for debugging, dump image to a bmp file
static void bmp_write(size_t image_height, size_t image_width, const void *image_data)
{
    FILE *bmp = fopen("font_debug.bmp", "wb");
    int pixel_data_size = image_width * image_height;
    int palette_size = 256 * 4;
    int file_size = 54 + palette_size + pixel_data_size;
    uint8_t header[54] = {'B',
                          'M',
                          file_size,
                          file_size >> 8,
    ...
    for (int i = 0; i < 256; ++i) {
        uint8_t entry[4] = {i,  // blue
                            i,  // green
                            i,  // red
                            0};

        fwrite(entry, 1, 4, bmp);
    }
    fwrite(image_data, 1, image_height * image_width, bmp);
    fclose(bmp);
    // grayscale palette
}
// This function does all the work.
static void Create_Font(const char *fontfile, size_t font_size, const char *outfile, int debug_image)
{
    // These are the characters that get stored. The last one ('\xFF')
    // indicates the picture used to draw 'unknown' characters.
    const char chars[] =
        "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890~!@#$%^&*()-=+;:'\",./?[]|\\ <>`\xFF";
    // Margins around characters to prevent them from 'bleeding' into
    // each other.
    size_t margin = 1;
    size_t image_height = 0, image_width = 16;
    size_t i;
    ...
    size_t x = 1, y;
    unsigned char *image;
    GlyphEntry entries[sizeof(chars) - 1];
    FILE *out;
    int pr;
    size_t imret;
    size_t approx_size = font_size * font_size * (sizeof(chars) - 1);
    size_t needed_min_width = sqrtf(approx_size * .6f);  // typical font width-to height ratio is .5 to .7
    while (image_width < needed_min_width) image_width *= 2;

    // This initializes FreeType
    FT_Library library;
    if (FT_Init_FreeType(&library) != 0) do_abort("Could not initialize FreeType2 library.");

    // Load the font
    FT_Face face;
    if (FT_New_Face(library, fontfile, 0, &face) != 0) do_abort("Could not load font file.");
    ...
    for (i = 0; i < (sizeof(chars) - 1); ++i) {
        ...
        int rows = (int)face->glyph->bitmap.rows;
        max_ascent = UC_MAX(face->glyph->bitmap_top, max_ascent);
        max_descent = UC_MAX(rows - face->glyph->bitmap_top, max_descent);
    }
    ...
    for (i = 0; i < (sizeof(chars) - 1); ++i) {
        ...
        for (row = 0; row < face->glyph->bitmap.rows; ++row) {
            for (pixel = 0; pixel < face->glyph->bitmap.width; ++pixel) {
                int x_ind = x + face->glyph->bitmap_left + pixel;
                int y_ind = (y - face->glyph->bitmap_top + row) * image_width;
                image[x_ind + y_ind] = face->glyph->bitmap.buffer[pixel + row * face->glyph->bitmap.width];
            }
        }
    ...
    if (pr != 4) do_abort("Error writing font header");

    // GlyphEntries
    for (i = 0; i < (sizeof(chars) - 1); ++i) {
        int ret;
        ret = pickle_glyphentry(out, &entries[i]);
        if (!ret) do_abort("Error writing font glyph entries :-(");
    }
    // Texture data
    imret = fwrite(image, 1, image_height * image_width, out);
    if (imret != (image_width * image_height)) do_abort("Error writing font data :-(");
...
int main(int args, char **argv)
{
    ...
    int debug_image = 0;
    if (args < 3) {
        printf("Usage %s ttf-font-file output file [font size] [debug]\n", argv[0]);
    } else {
        if (args > 3) {
            size_t arg_size = atoi(argv[3]);
            if (arg_size != 0) size = arg_size;
        }
        if (args == 5) {
            debug_image = 1;
        }
        Create_Font(argv[1], size, argv[2], debug_image);
        return 0;
    }
