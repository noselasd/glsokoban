...
static void draw_wall(GameData *game, int x, int y)
{
    material_apply(&m_normal);
    glBindTexture(GL_TEXTURE_2D, game->textures[Brick]);
    draw_cube(x, y);
...
static void glDrawLevel(GameData *game, Level *l)
{
    int x, y;
...
{
    static const char *const finish_texts[] = {"Level Finished!", "Congratulations!", "You made it !!", "Well Done !!",
                                               "Goal! Goal!"};
...
void frame_begin(GameData *game, int finished)
{
    ...
    glDrawLevel(game, &game->currentLevel);
    start2d();
    glColor3f(0.9f, 0.9f, 0.9f);
    ftPrint("glSokoban", 0.01f, 0.01f, 0.028f);
    snprintf(movestr, sizeof(movestr) - 1, "Moves: %d", game->nr_moves);
    ftPrint(movestr, 0.01, 0.95, 0.028);
    if (finished) ftPrint(finish_text, 0.40f, 0.50f, 0.048f);
...
void calc_distance(GameData *game)
{
    ...
    if (game->currentLevel.height >= game->currentLevel.width)
        max = game->currentLevel.height;
    else
